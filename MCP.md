...
TripNest includes a built-in [Model Context Protocol](https://modelcontextprotocol.io/) (MCP) server that lets AI
assistants — such as Claude Desktop, Cursor, or any MCP-compatible client — read and modify your trip data through a
structured API.

> **Note:** MCP is an addon that must be enabled by your TripNest administrator before it becomes available.
...
```json
{
  "mcpServers": {
    "tripnest": {
      ...
      "args": [
        "mcp-remote",
        "https://your-tripnest-instance.com/mcp",
        "--header",
        "Authorization: Bearer tripnest_your_token_here"
      ]
    }
  }
}
...
| **Demo mode restrictions**              | If TripNest is running in demo mode, all write operations through MCP are blocked.                                                                   |
...
Resources provide read-only access to your TripNest data. MCP clients can read these to understand the current state before
making changes.

| Resource          | URI                                        | Description                                               |
|-------------------|--------------------------------------------|-----------------------------------------------------------|
| Trips             | `tripnest://trips`                             | All trips you own or are a member of                      |
| Trip Detail       | `tripnest://trips/{tripId}`                    | Single trip with metadata and member count                |
| Days              | `tripnest://trips/{tripId}/days`               | Days of a trip with their assigned places                 |
| Places            | `tripnest://trips/{tripId}/places`             | All places/POIs saved in a trip                           |
| Budget            | `tripnest://trips/{tripId}/budget`             | Budget and expense items                                  |
| Packing           | `tripnest://trips/{tripId}/packing`            | Packing checklist                                         |
| Reservations      | `tripnest://trips/{tripId}/reservations`       | Flights, hotels, restaurants, etc.                        |
| Day Notes         | `tripnest://trips/{tripId}/days/{dayId}/notes` | Notes for a specific day                                  |
| Accommodations    | `tripnest://trips/{tripId}/accommodations`     | Hotels/rentals with check-in/out details                  |
| Members           | `tripnest://trips/{tripId}/members`            | Owner and collaborators                                   |
| Collab Notes      | `tripnest://trips/{tripId}/collab-notes`       | Shared collaborative notes                                |
| Categories        | `tripnest://categories`                        | Available place categories (for use when creating places) |
| Bucket List       | `tripnest://bucket-list`                       | Your personal travel bucket list                          |
| Visited Countries | `tripnest://visited-countries`                 | Countries marked as visited in Atlas                      |

---

## Tools (read-write)

TripNest exposes **34 tools** organized by feature area. Use `get_trip_summary` as a starting point — it returns everything
...
Before writing anything to TripNest, do some research: look up what's worth
...
Once you have a solid plan, write the whole thing to TripNest:
- Create the trip
...
PDF of the generated trip: [./docs/TripNest-Generated-by-MCP.pdf](./docs/TripNest-Generated-by-MCP.pdf)
