...
export default function DashboardPage(): React.ReactElement {
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'list'>(() => (localStorage.getItem('tripnest_dashboard_view') as 'grid' | 'list') || 'grid')
  const [deleteTrip, setDeleteTrip] = useState<DashboardTrip | null>(null)

  const toggleViewMode = () => {
    setViewMode(prev => {
      const next = prev === 'grid' ? 'list' : 'grid'
      localStorage.setItem('tripnest_dashboard_view', next)
      return next
    })
  }
