...
const MFA_BACKUP_SESSION_KEY = 'tripnest_mfa_backup_codes_pending'
interface McpToken {
  id: number
  name: string
  token_prefix: string
...
export default function SettingsPage(): React.ReactElement {
  ...
  const downloadBackupCodes = (): void => {
    ...
    a.href = url
    a.download = 'tripnest-mfa-backup-codes.txt'
  ...
  const printBackupCodes = (): void => {
    if (!backupCodesText) return
    const html = `<!doctype html><html><head><meta charset="utf-8"/><title>TripNest MFA Backup Codes</title>
      <style>body{font-family:Arial,sans-serif;padding:32px}h1{font-size:20px}pre{font-size:16px;line-height:1.6}</style>
      </head><body><h1>TripNest MFA Backup Codes</h1><p>${new Date().toLocaleString()}</p><pre>${backupCodesText}</pre></body></html>`
            ...
            <Section title={t('settings.about')} icon={Info}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: 'var(--bg-tertiary)', borderRadius: 99, padding: '6px 14px' }}>
                  <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-secondary)' }}>TripNest</span>
                ...
                <a href={AURESTUDIO_WEB} target="_blank" rel="noopener noreferrer"
                  style={{
                    ...
                    border: '1px solid var(--border-secondary)', fontFamily: 'inherit',
                  }}
