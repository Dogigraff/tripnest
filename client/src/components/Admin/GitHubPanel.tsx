...
import { Tag, Calendar, ExternalLink, ChevronDown, ChevronUp, Loader2, Globe, Send } from 'lucide-react'
import { getLocaleForLanguage, useTranslation } from '../../i18n'
import apiClient from '../../api/client'
import { AURESTUDIO_TELEGRAM, AURESTUDIO_WEB } from '../../constants/aurestudio'

const REPO = 'tripnest/tripnest'
const PER_PAGE = 10
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <a
          ...
          onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--text-muted)'; e.currentTarget.style.boxShadow = '0 0 0 1px #00000014' }}
        ...
        >
          ...
          <div>
            <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>AureStudio</div>
            <div className="text-xs" style={{ color: 'var(--text-faint)' }}>{t('admin.github.support')}</div>
          </div>
          <ExternalLink size={14} className="ml-auto flex-shrink-0" style={{ color: 'var(--text-faint)' }} />
        </a>
        <a
          href={AURESTUDIO_TELEGRAM}
          target="_blank"
          rel="noopener noreferrer"
          className="rounded-xl border overflow-hidden flex items-center gap-4 px-5 py-4 transition-all"
          style={{ background: 'var(--bg-card)', borderColor: 'var(--border-primary)', textDecoration: 'none' }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = '#229ED9'; e.currentTarget.style.boxShadow = '0 0 0 1px #229ED922' }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--border-primary)'; e.currentTarget.style.boxShadow = 'none' }}
        >
          <div style={{ width: 40, height: 40, borderRadius: 10, background: '#229ED915', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            <Send size={20} style={{ color: '#229ED9' }} />
          </div>
          <div>
            <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>Telegram</div>
            <div className="text-xs" style={{ color: 'var(--text-faint)' }}>{t('admin.github.support')}</div>
          </div>
          <ExternalLink size={14} className="ml-auto flex-shrink-0" style={{ color: 'var(--text-faint)' }} />
        </a>
