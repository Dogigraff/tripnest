...
import { AURESTUDIO_TELEGRAM, AURESTUDIO_WEB } from '../../constants/aurestudio'
...
interface NavbarProps {
  ...
  showBack?: boolean
  onShare?: () => void
  /** Extra controls on trip pages (e.g. AI Planner), shown before Share */
  tripExtra?: React.ReactNode
}
...
export default function Navbar({ tripTitle, tripId, onBack, showBack, onShare, tripExtra }: NavbarProps): React.ReactElement {
  ...
  return (
    <nav style={{
      ...
      height: 'var(--nav-h)',
    }} className="flex items-center px-4 gap-2 sm:gap-4 fixed top-0 left-0 right-0 z-[200]">
      {/* Left side */}
      <div className="flex items-center gap-3 min-w-0">
        ...
        <Link to="/dashboard" className="flex items-center transition-colors flex-shrink-0">
          <img src={dark ? '/logo-dark.svg' : '/logo-light.svg'} style={{ height: '32px' }} alt="TripNest" />
        </Link>
                    ...
                    <div className="px-4 pt-2 pb-2.5 text-center" style={{ marginTop: 4, borderTop: '1px solid var(--border-secondary)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, flexWrap: 'wrap' }}>
                        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: 'var(--bg-tertiary)', borderRadius: 99, padding: '4px 12px' }}>
                          <img src={dark ? '/text-light.png' : '/text-dark.png'} alt="TripNest" style={{ height: 10, width: 'auto', opacity: 0.5 }} />
                          <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-faint)' }}>v{appVersion}</span>
                        </div>
                        <a href={AURESTUDIO_WEB} target="_blank" rel="noopener noreferrer"
                          style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-faint)', textDecoration: 'none', padding: '4px 8px', borderRadius: 8, background: 'var(--bg-tertiary)' }}
                          onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-hover)' }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'var(--bg-tertiary)' }}
                          title="AureStudio">
                          AureStudio
                        </a>
                        <a href={AURESTUDIO_TELEGRAM} target="_blank" rel="noopener noreferrer"
                          style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-faint)', textDecoration: 'none', padding: '4px 8px', borderRadius: 8, background: 'var(--bg-tertiary)' }}
                          onMouseEnter={e => { e.currentTarget.style.background = 'var(--bg-hover)' }}
                          onMouseLeave={e => { e.currentTarget.style.background = 'var(--bg-tertiary)' }}
                          title="Telegram">
