...
vi.mock('../../src/config', () => ({
  JWT_SECRET: 'test-jwt-secret-for-tripnest-testing-only',
...
describe('Login', () => {
  it('AUTH-001 — successful login returns 200, user object, and tripnest_session cookie', async () => {
    ...
    const cookies: string[] = Array.isArray(res.headers['set-cookie'])
      ...
      : [res.headers['set-cookie']];
    expect(cookies.some((c: string) => c.includes('tripnest_session'))).toBe(true);
  });
  ...
  it('AUTH-013 — POST /api/auth/logout clears session cookie', async () => {
    ...
    const sessionCookie = cookies.find((c: string) => c.includes('tripnest_session'));
...
describe('Demo login', () => {
  ...
  it('AUTH-022 — POST /api/auth/demo-login with DEMO_MODE and demo user returns 200 + cookie', async () => {
    testDb.prepare(
      "INSERT INTO users (username, email, password_hash, role) VALUES ('demo', 'demo@tripnest.app', 'x', 'user')"
    ).run();
    process.env.DEMO_MODE = 'true';
    try {
      const res = await request(app).post('/api/auth/demo-login');
      expect(res.status).toBe(200);
      expect(res.body.user.email).toBe('demo@tripnest.app');
    } finally {
      delete process.env.DEMO_MODE;
    }
  });
