...
describe('buildWebhookBody', () => {
  ...
  it('Discord embed footer defaults to TripNest when no trip name', () => {
    ...
    expect(body.embeds[0].footer.text).toBe('TripNest');
  });
  ...
  it('generic URL produces plain JSON with original fields plus timestamp and source', () => {
    ...
    expect(body).toHaveProperty('source', 'TripNest');
  });
});
...
describe('buildEmailHtml', () => {
  ...
  it('uses English i18n strings for lang=en', () => {
    ...
    expect(html).toContain('notifications enabled in TripNest');
  });

  it('uses German i18n strings for lang=de', () => {
    const html = buildEmailHtml('Subject', 'Body', 'de');
    expect(html).toContain('TripNest aktiviert');
  });
  ...
  it('falls back to English i18n for unknown language', () => {
    ...
    expect(unknown).toContain('notifications enabled in TripNest');
  });
});
