...
describe('extractToken', () => {
  it('returns cookie value when tripnest_session cookie is set', () => {
    const req = makeReq({ cookies: { tripnest_session: 'cookie-token' } });
  ...
  it('prefers cookie over Authorization header when both are present', () => {
    const req = makeReq({
      cookies: { tripnest_session: 'cookie-token' },
...
describe('authenticate', () => {
  ...
  it('returns 401 when JWT is invalid', () => {
    ...
    authenticate(makeReq({ cookies: { tripnest_session: 'invalid.jwt.token' } }), res, next);
    expect(next).not.toHaveBeenCalled();
    expect(status).toHaveBeenCalledWith(401);
  });
});
