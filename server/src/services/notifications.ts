...
interface EmailStrings { footer: string; manage: string; madeWith: string; openTripNest: string }

const I18N: Record<string, EmailStrings> = {
  en: { footer: 'You received this because you have notifications enabled in TripNest.', manage: 'Manage preferences in Settings', madeWith: 'Made with', openTripNest: 'Open TripNest' },
  de: { footer: 'Du erhältst diese E-Mail, weil du Benachrichtigungen in TripNest aktiviert hast.', manage: 'Einstellungen verwalten', madeWith: 'Made with', openTripNest: 'TripNest öffnen' },
  fr: { footer: 'Vous recevez cet e-mail car les notifications sont activées dans TripNest.', manage: 'Gérer les préférences', madeWith: 'Made with', openTripNest: 'Ouvrir TripNest' },
  es: { footer: 'Recibiste esto porque tienes las notificaciones activadas en TripNest.', manage: 'Gestionar preferencias', madeWith: 'Made with', openTripNest: 'Abrir TripNest' },
  nl: { footer: 'Je ontvangt dit omdat je meldingen hebt ingeschakeld in TripNest.', manage: 'Voorkeuren beheren', madeWith: 'Made with', openTripNest: 'TripNest openen' },
  ru: { footer: 'Вы получили это, потому что у вас включены уведомления в TripNest.', manage: 'Управление настройками', madeWith: 'Made with', openTripNest: 'Открыть TripNest' },
  zh: { footer: '您收到此邮件是因为您在 TripNest 中启用了通知。', manage: '管理偏好设置', madeWith: 'Made with', openTripNest: '打开 TripNest' },
  ar: { footer: 'تلقيت هذا لأنك قمت بتفعيل الإشعارات في TripNest.', manage: 'إدارة التفضيلات', madeWith: 'Made with', openTripNest: 'فتح TripNest' },
};
...
const EVENT_TEXTS: Record<string, Record<EventType, EventTextFn>> = {
  en: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion Invite', body: `${p.actor} invited you to fuse vacation plans. Open TripNest to accept or decline.` }),
  ...
  de: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion-Einladung', body: `${p.actor} hat dich eingeladen, Urlaubspläne zu fusionieren. Öffne TripNest um anzunehmen oder abzulehnen.` }),
  ...
  fr: {
    ...
    vacay_invite: p => ({ title: 'Invitation Vacay Fusion', body: `${p.actor} vous invite à fusionner les plans de vacances. Ouvrez TripNest pour accepter ou refuser.` }),
  ...
  es: {
    ...
    vacay_invite: p => ({ title: 'Invitación Vacay Fusion', body: `${p.actor} te invitó a fusionar planes de vacaciones. Abre TripNest para aceptar o rechazar.` }),
  ...
  nl: {
    ...
    vacay_invite: p => ({ title: 'Vacay Fusion uitnodiging', body: `${p.actor} nodigt je uit om vakantieplannen te fuseren. Open TripNest om te accepteren of af te wijzen.` }),
  ...
  ru: {
    ...
    vacay_invite: p => ({ title: 'Приглашение Vacay Fusion', body: `${p.actor} приглашает вас объединить планы отпуска. Откройте TripNest для подтверждения.` }),
  ...
  zh: {
    ...
    vacay_invite: p => ({ title: 'Vacay 融合邀请', body: `${p.actor} 邀请你合并假期计划。打开 TripNest 接受或拒绝。` }),
  ...
  ar: {
    ...
    vacay_invite: p => ({ title: 'دعوة دمج الإجازة', body: `${p.actor} يدعوك لدمج خطط الإجازة. افتح TripNest للقبول أو الرفض.` }),
    ...
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width: 480px; background: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 24px rgba(0,0,0,0.06);">
        ...
        ${appUrl ? `<tr><td style="padding: 8px 32px 32px; text-align: center;">
          <a href="${ctaHref}" style="display: inline-block; padding: 12px 28px; background: #111827; color: #ffffff; font-size: 13px; font-weight: 600; text-decoration: none; border-radius: 10px; letter-spacing: 0.2px;">${s.openTripNest}</a>
        </td></tr>` : ''}
        <!-- Footer -->
        <tr><td style="padding: 20px 32px; background: #f9fafb; border-top: 1px solid #f3f4f6; text-align: center;">
          ...
          <p style="margin: 0; font-size: 10px; color: #d1d5db;">${s.madeWith} <span style="color: #ef4444;">&hearts;</span> by Maurice &middot; <a href="https://github.com/tripnest/tripnest" style="color: #9ca3af; text-decoration: none;">GitHub</a></p>
        </td></tr>
      </table>
    </td></tr>
  </table>
...
export function buildWebhookBody(url: string, payload: { event: string; title: string; body: string; tripName?: string }): string {
  ...
  if (isDiscord) {
    return JSON.stringify({
      embeds: [{
        ...
        color: 0x3b82f6,
        footer: { text: payload.tripName ? `Trip: ${payload.tripName}` : 'TripNest' },
  ...
  return JSON.stringify({ ...payload, timestamp: new Date().toISOString(), source: 'TripNest' });
}
...
export async function testSmtp(to: string): Promise<{ success: boolean; error?: string }> {
  try {
    const sent = await sendEmail(to, 'Test Notification', 'This is a test email from TripNest. If you received this, your SMTP configuration is working correctly.');
...
export async function testWebhook(): Promise<{ success: boolean; error?: string }> {
  try {
    const sent = await sendWebhook({ event: 'test', title: 'Test Notification', body: 'This is a test webhook from TripNest. If you received this, your webhook configuration is working correctly.' });
