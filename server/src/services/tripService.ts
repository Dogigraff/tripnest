...
export function exportICS(tripId: string | number): { ics: string; filename: string } {
  ...
  const uid = (id: number, type: string) => `tripnest-${type}-${id}@tripnest`;
  ...
  let ics = 'BEGIN:VCALENDAR\r\nVERSION:2.0\r\nPRODID:-//TripNest//Travel Planner//EN\r\nCALSCALE:GREGORIAN\r\nMETHOD:PUBLISH\r\n';
  ics += `X-WR-CALNAME:${esc(trip.title || 'TripNest Trip')}\r\n`;
  ...
  const safeFilename = (trip.title || 'tripnest-trip').replace(/["\r\n]/g, '').replace(/[^\w\s.-]/g, '_');
