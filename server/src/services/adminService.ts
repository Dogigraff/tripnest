...
export async function getGithubReleases(perPage: string = '10', page: string = '1') {
  try {
    const resp = await fetch(
      `https://api.github.com/repos/tripnest/tripnest/releases?per_page=${perPage}&page=${page}`,
      { headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'TripNest-Server' } }
    );
    if (!resp.ok) return [];
...
export async function checkVersion() {
  ...
  try {
    const resp = await fetch(
      'https://api.github.com/repos/tripnest/tripnest/releases/latest',
      { headers: { 'Accept': 'application/vnd.github.v3+json', 'User-Agent': 'TripNest-Server' } }
    );
