<p align="center">
  <picture>
    ...
    <img src="client/public/logo-light.svg" alt="TripNest" height="60" />
  </picture>
  <br />
...
<p align="center">
  ...
  <a href="https://hub.docker.com/r/tripnest/tripnest"><img src="https://img.shields.io/docker/pulls/tripnest/tripnest" alt="Docker Pulls" /></a>
  <a href="https://github.com/tripnest/tripnest"><img src="https://img.shields.io/github/stars/tripnest/tripnest" alt="GitHub Stars" /></a>
  <a href="https://github.com/tripnest/tripnest/commits"><img src="https://img.shields.io/github/last-commit/tripnest/tripnest" alt="Last Commit" /></a>
</p>

<p align="center">
  A self-hosted, real-time collaborative travel planner with interactive maps, budgets, packing lists, and more.
  <br />
  <strong><a href="https://demo.pakulat.org">Live Demo</a></strong> — Try TripNest without installing. Resets hourly.
</p>

![TripNest Screenshot](docs/screenshot.png)
![TripNest Screenshot 2](docs/screenshot-2.png)
...
- **PDF Export** — Export complete trip plans as PDF with cover page, images, notes, and TripNest branding
...
ENCRYPTION_KEY=$(openssl rand -hex 32) docker run -d -p 3000:3000 \
  ...
  -v ./data:/app/data -v ./uploads:/app/uploads tripnest/tripnest
```
...
TripNest works as a Progressive Web App — no App Store needed:

1. Open your TripNest instance in the browser (HTTPS required)
2. **iOS**: Share button → "Add to Home Screen"
3. **Android**: Menu → "Install app" or "Add to Home Screen"
4. TripNest launches fullscreen with its own icon, just like a native app
...
services:
  app:
    image: tripnest/tripnest:latest
    container_name: tripnest
    read_only: true
    security_opt:
    ...
    environment:
      ...
      # - OIDC_CLIENT_ID=tripnest # OpenID Connect client ID
      # - OIDC_CLIENT_SECRET=supersecret # OpenID Connect client secret
      # - OIDC_DISPLAY_NAME=SSO # Label shown on the SSO login button
      # - OIDC_ONLY=false # Set to true to disable local password auth entirely (SSO only)
      # - OIDC_ADMIN_CLAIM=groups # OIDC claim used to identify admin users
      # - OIDC_ADMIN_VALUE=app-tripnest-admins # Value of the OIDC claim that grants admin role
      ...
      # - ADMIN_EMAIL=admin@tripnest.local # Initial admin e-mail — only used on first boot when no users exist
...
```bash
docker pull tripnest/tripnest
docker rm -f tripnest
docker run -d --name tripnest -p 3000:3000 -v ./data:/app/data -v ./uploads:/app/uploads --restart unless-stopped tripnest/tripnest
```

> **Tip:** Not sure which paths you used? Run `docker inspect tripnest --format '{{json .Mounts}}'` before removing the container.
...
```bash
docker exec -it tripnest node --import tsx scripts/migrate-encryption.ts
```
...
For production, put TripNest behind a reverse proxy with HTTPS (e.g. Nginx, Caddy, Traefik).

> **Important:** TripNest uses WebSockets for real-time sync. Your reverse proxy must support WebSocket upgrades on the `/ws` path.
...
server {
    listen 80;
    server_name tripnest.yourdomain.com;
...
| `APP_URL` | Public base URL of this instance (e.g. `https://tripnest.example.com`). Required when OIDC is enabled — must match the redirect URI registered with your IdP. Also used as the base URL for external links in email notifications. | — |
| **OIDC / SSO** | | |
...
| `OIDC_DISCOVERY_URL` | Override the auto-constructed OIDC discovery endpoint. Useful for providers that expose it at a non-standard path (e.g. Authentik: `https://auth.example.com/application/o/tripnest/.well-known/openid-configuration`) | — |
| **Initial Setup** | | |
| `ADMIN_EMAIL` | Email for the first admin account created on initial boot. Must be set together with `ADMIN_PASSWORD`. If either is omitted a random password is generated and printed to the server log. Has no effect once any user exists. | `admin@tripnest.local` |
...
4. In TripNest: Admin Panel → Settings → Google Maps

## Building from Source

```bash
git clone https://github.com/tripnest/tripnest.git
cd tripnest
...
- **Logs**: `./data/logs/tripnest.log` (auto-rotated)
